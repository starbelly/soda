...
enif_crypto_generichash(ErlNifEnv * env, int argc, ERL_NIF_TERM const argv[])
{
	...
	if (0 != crypto_generichash(h.data, h.size, m.data, m.size, k.data, k.size)) {
		FREE_BIN(&h);
			     ...
			     ERL_NIF_TERM const argv[])
{
	...
	crypto_generichash_state *state = (crypto_generichash_state *)ALLOC_RESOURCE(gen_hash_state_t, b);
